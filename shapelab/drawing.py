...
class Drawing(Serializable):
    ...
    def total_area(self) -> float:
        ...
        Почему здесь переполнение — не ошибка, хотя конструкторы фигур его
        запрещают: фигура — это данные, которые хранятся, сравниваются и
        записываются в JSON, поэтому каждая её величина обязана быть
        представима числом. Общая площадь ничего не хранит — это величина
        только для вывода, вычисляемая из корректных фигур. Чертёж из двух
        допустимых квадратов со стороной 1e154 тоже допустим, и отказаться
        его показывать было бы хуже, чем честно ответить «больше 1e308».
        """
        try:
