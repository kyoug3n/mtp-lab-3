...
Если сумма больше 1e308, `total_area()` возвращает `inf`, а меню пишет «Общая площадь:
больше 1e308». Конструкторы фигур переполнение, наоборот, запрещают, и это
осознанное различие: фигура — это данные, которые хранятся, сравниваются и
записываются в JSON, поэтому каждая её величина обязана быть представима числом.
Общая площадь ничего не хранит — это величина только для вывода, вычисляемая из
корректных фигур. Чертёж из двух допустимых квадратов со стороной 1e154 тоже
допустим, и отказаться его показывать было бы хуже, чем честно ответить «больше 1e308».
